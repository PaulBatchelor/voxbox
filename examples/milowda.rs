...
fn main() {
    ...
    clk.set_freq(110.0 / 60.0);
    let mut dcblk = DCBlocker::new(sr);
    let mut delay = Delay::new(sr, 0.2);
    ...
    let mut wav = MonoWav::new("milowda.wav");
    ...
    bass.glottis.set_shape(0.2);
    bass.glottis.srand(54321);
    bass.glottis.set_aspiration(0.04);

    alto.tract.drm(&shape_ah_alto);
    alto.vibrato_rate(6.0);
    alto.vibrato_depth(0.1);
    alto.glottis.set_shape(0.3);
    alto.glottis.set_aspiration(0.05);
    ...
    for _ in 0..(sr as f32 * 50.0) as usize {
        ...
        let rvb = delay.tick(rvb);
        //let out = sum;
        let out = sum + rvb * db2lin(-10.);
